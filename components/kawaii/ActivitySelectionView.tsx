...
import type { KawaiiTripSuggestions } from '@/types/kawaii';
import { buildItinerary } from '@/services/api';
...
const ActivitySelectionView = ({ suggestions }: { suggestions: KawaiiTripSuggestions }) => {
  const [activeCity, setActiveCity] = useState(0);
  const [building, setBuilding] = useState(false);
  const userId = useUserStore((s) => s.userId);
  const { selectedActivityIds, planningStep, setPlanningStep, setItinerary, setConflicts } = useTripStore();
  const selectedCount = selectedActivityIds.length;

  const handleBuild = async () => {
    if (selectedCount === 0 || building) return;
    setBuilding(true);
    try {
      const result = await buildItinerary(userId ?? 'demo', selectedActivityIds, suggestions);
  ...
  return (
    <motion.div className="flex flex-col h-full" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={spring}>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <motion.h2
              ...
              initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, ...spring }}
            >
        ...
        {suggestions.cities.map((city, i) => (
          <motion.button
            ...
            className={`kawaii-pill text-xs whitespace-nowrap flex items-center gap-1.5 transition-colors ${
              activeCity === i ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground'
            }`}
      ...
      <motion.div
        ...
        initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4, ...spring }}
      >
        <div className="glass-card px-5 py-4 flex items-center justify-between max-w-2xl mx-auto" style={{ borderRadius: '20px' }}>
          <div>
            <span className="text-sm font-bold font-heading">{selectedCount} selected</span>
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }}
            style={{ opacity: selectedCount > 0 && !building ? 1 : 0.5 }}
            disabled={building || selectedCount === 0}
          >
            {building ? 'Building…' : 'Build My Trip'}
