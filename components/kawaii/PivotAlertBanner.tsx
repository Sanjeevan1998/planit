...
import type { KawaiiPivotAlert } from '@/types/kawaii';
...
interface Props { alert: KawaiiPivotAlert; onViewAlternatives: () => void; onDismiss: () => void; }
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ y: 0, opacity: 1, rotate: [0, -1, 1, -0.5, 0] }}
        exit={{ y: -100, opacity: 0 }}
        transition={spring}
      >
        <div className="glass-card px-4 py-3 flex items-center gap-3 max-w-2xl mx-auto" style={{ borderRadius: '20px', borderLeft: `4px solid ${style.bg}` }}>
          <motion.div
          ...
          <motion.button onClick={onViewAlternatives} className="kawaii-pill bg-foreground text-background text-xs flex items-center gap-1.5 flex-shrink-0" whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }}>
            View <ArrowRight className="w-3 h-3" />
          </motion.button>
          <motion.button onClick={onDismiss} className="w-7 h-7 rounded-full bg-muted flex items-center justify-center flex-shrink-0" whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
