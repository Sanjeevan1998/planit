...
import type { KawaiiDayPlan, KawaiiItineraryNode } from '@/types/kawaii';
import ItineraryNodeCard from './ItineraryNodeCard';
import NodeDetailDialog from './NodeDetailDialog';

const spring = { type: 'spring' as const, stiffness: 300, damping: 28 };

const ItineraryTimeline = ({ days }: { days: KawaiiDayPlan[] }) => {
  const [selectedNode, setSelectedNode] = useState<KawaiiItineraryNode | null>(null);
            ...
            <div className="relative pl-6 ml-4 space-y-3">
              ...
              {day.nodes.map((node, i) => (
                <div key={node.id} className="relative">
                  ...
                  />
                  <ItineraryNodeCard node={node} index={dayIdx * 4 + i} onClick={() => setSelectedNode(node)} />
                </div>
              ))}
            </div>
          </motion.div>
