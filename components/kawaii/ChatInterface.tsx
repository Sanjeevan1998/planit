...
const ChatInterface = () => {
  ...
  // Open conflict modal when conflicts appear
  useEffect(() => {
    if (conflicts.length > 0) setShowConflict(true);
  }, [conflicts]);
  ...
  const handleSend = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await sendChatMessage(text, userId ?? 'demo', itinerary?.id);
      addMessage({ role: 'assistant', content: result.response });
      if (result.mode === 'suggest' && result.tripSuggestions) {
        setSuggestions(result.tripSuggestions);
        setTimeout(() => setPlanningStep('review'), 800);
      }
    } catch {
  ...
  }, [input, isLoading, userId, itinerary?.id, addMessage, setLoading, setSuggestions, setPlanningStep]);

  // ── Routing by planning step ────────────────────────────────────────────────
    ...
    return (
      <>
        {pivotAlert && (
          <PivotAlertBanner
            alert={pivotAlert}
            onViewAlternatives={() => setPivotAlert(null)}
            onDismiss={() => setPivotAlert(null)}
          />
        )}
  ...
  if (planningStep === 'customize' && foodSuggestions.length > 0) {
    return <FoodDiscoveryView foods={foodSuggestions} />;
  }
        ...
        {showConflict && conflicts.length > 0 && (
          <ConflictResolutionModal
            ...
            onClose={() => {
              ...
              if (conflicts.length <= 1) setShowConflict(false);
            }}
          />
        )}
  ...
  // Default: chat view
  return (
    <motion.div className="flex flex-col h-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.4 }}>
      <motion.div
        className="px-4 sm:px-6 pt-6 pb-3"
        initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, ...spring }}
      >
            ...
            >
              <div
                ...
                style={{
                  ...
                  background: msg.role === 'user' ? 'hsl(259 100% 85% / 0.4)' : 'rgba(255,255,255,0.85)',
      ...
      <motion.div
        ...
        initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, ...spring }}
      >
        <div className="glass-card flex items-center gap-2 px-4 py-2" style={{ borderRadius: '999px' }}>
          <motion.button
            onClick={() => setIsRecording(!isRecording)}
            className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 transition-colors ${isRecording ? 'bg-accent text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
