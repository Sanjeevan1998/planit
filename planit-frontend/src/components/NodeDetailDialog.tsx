...
import { X, MapPin, Clock, Star, ExternalLink, Sparkles, Train, Car } from 'lucide-react';
      ...
      {node && (
        <>
          ...
          <motion.div
            ...
            transition={spring}
          >
            <div className="glass-card overflow-hidden" style={{ borderRadius: '24px', border: '1px solid hsl(var(--border))' }}>
              {/* Image */}
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {node.tags.map(tag => (
                    <span key={tag} className="text-[10px] font-body text-muted-foreground bg-muted px-2.5 py-1 rounded-pill">
                      {tag}
                    </span>
                  ))}
                </div>
                ...
                {node.transport_options && node.transport_options.length > 0 && (
                  <div className="space-y-2">
                    ...
                    {node.transport_options.map((t, i) => {
                      const ModeIcon = modeIcons[t.mode] || Train;
                      return (
