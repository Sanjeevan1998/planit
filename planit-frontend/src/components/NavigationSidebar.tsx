...
const NavigationSidebar = () => {
  ...
  return (
    <>
      {/* Desktop: Minimal icon strip */}
      <aside className="hidden md:flex flex-col items-center w-16 h-screen fixed left-0 top-0 z-30 py-6 gap-2"
          ...
          {navItems.map((item, i) => (
            <motion.button
              ...
              className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors
                ${i === 0 ? 'bg-foreground/10 text-foreground' : 'text-muted-foreground hover:bg-foreground/5 hover:text-foreground'}`}
      ...
      {/* Mobile: Floating toggle + overlay menu */}
      <motion.button
        ...
        >
          ...
          >
            {navItems.map((item, i) => (
              <button
                key={item.label}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold font-body transition-colors
                  ${i === 0 ? 'bg-foreground/8 text-foreground' : 'text-muted-foreground hover:bg-foreground/5'}`}
              >
                <item.icon className="w-[18px] h-[18px]" />
