...
import { FoodSuggestion } from '@/types/trip';
...
const FoodCard = ({ item }: { item: FoodSuggestion }) => {
  ...
  return (
    <motion.div
      ...
      animate={{
        ...
        boxShadow: selected
          ? '0 8px 32px rgba(255, 214, 192, 0.4), 0 0 0 2px hsl(22 100% 87%)'
          : 'var(--shadow-pastel)',
      }}
    ...
    >
      {isImage && (
        <div className="w-full h-32 overflow-hidden" style={{ borderRadius: '18px 18px 0 0' }}>
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-white/50 to-transparent" style={{ borderRadius: '18px 18px 0 0' }} />
        </div>
      )}
      ...
      <div className="p-3.5">
        <div className="flex items-center gap-2 mb-1.5">
          <span
            ...
            style={{ backgroundColor: budgetColors[item.budget_tier] }}
          >
            {item.meal_type}
          </span>
...
const FoodDiscoveryView = ({ foods }: { foods: FoodSuggestion[] }) => {
  const [filter, setFilter] = useState<typeof mealFilters[number]>('all');
  const { selectedFoodIds, planningStep, setPlanningStep } = useTripStore();
    ...
    >
      ...
      <div className="px-4 sm:px-6 flex gap-2 overflow-x-auto pb-3 scrollbar-hide">
        {mealFilters.map((m) => (
          <motion.button
            key={m}
            onClick={() => setFilter(m)}
            className={`kawaii-pill text-xs capitalize whitespace-nowrap transition-colors ${
              filter === m ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground'
            }`}
      ...
      >
        <div className="glass-card px-5 py-4 flex items-center justify-between max-w-2xl mx-auto" style={{ borderRadius: '20px' }}>
          ...
          <motion.button
            onClick={() => setPlanningStep('finalize')}
